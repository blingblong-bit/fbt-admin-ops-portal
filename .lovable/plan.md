...
3. **Safety proof.** Before and after the run, the report takes row counts plus a checksum of every row's contents and the latest update time. This covers clients, payments, client activity, dues messages, renewal campaigns and renewal messages. A matching checksum shows no row was edited, not just that none were added or removed. The report also confirms 0 messages sent, 0 campaigns touched, 0 data written, and both texting switches OFF.
...
- Live spot check: a temporary admin-guarded read-only endpoint, deleted right after use. Before and after, for each of `clients`, `square_payments`, `client_activities`, `dues_messages`, `renewal_campaigns`, `renewal_messages`, the check records `count(*)`, `md5(string_agg(t::text, '' order by id))` and `max(updated_at)` where that column exists. All three must match. Checks that `SMS_DUES_SENDING_ENABLED` and `RENEWAL_AUTO_TEXT_ENABLED` are not "true".
