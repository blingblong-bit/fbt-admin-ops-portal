ALTER TABLE public.clients ADD COLUMN IF NOT EXISTS previous_package_owed numeric NOT NULL DEFAULT 0;
...
BEGIN
  ...
  SELECT amount_paid, previous_package_owed INTO v_current_paid, v_prev_owed
  FROM public.clients
  ...
  -- Oldest debt first: pay down the previous package, remainder to current.
  v_to_prev := LEAST(v_prev_owed, v_amount_dollars);
  v_to_current := v_amount_dollars - v_to_prev;
